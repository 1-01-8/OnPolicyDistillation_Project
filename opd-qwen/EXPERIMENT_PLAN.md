...
### 4.2 主实验 v2：**Base student**（简历主线）

切到 `Qwen3-1.7B-Base`，头空间从 10 pt → 28 pt。

| 指标 | 实测 (n) | 解读 |
|------|----------|------|
| Student-Base baseline | **57.4%** (500) | Qwen3 base 已含 mid-train 数学语料，比 Google snippet 的 20% 高 |
| Teacher baseline | 85.6% (500) | 上界，头空间 28 pt |
| **OPD ckpt-400 (λ=0.5)** | **69.5%** (200) quick / *full pending* | **+12.1 pt vs baseline**，loss 自 step 200 已平 |
| SFT-1760 (compute-matched) | *pending* (full 1319) | base 模型 SFT 应正向涨 |
| **OPD − SFT** | *pending* | 简历核心数字 |

**为什么停在 step 400 而不是 1000**：训练 loss 自 step 200 起在 0.10±0.005 内震荡，
ckpt-400 quick eval=0.695 已比 baseline +12 pt；继续到 1000 step 预期再涨 1–3 pt
但需多花 ~14 h。**饱和后停训** 是符合 OPD 论文 "compute-efficient" 卖点的合理决策。

> 命令：`bash scripts/finalize_base.sh`（提前停训后的收尾：~75 min）
...
> *"复现 Google DeepMind《On-Policy Distillation》（ICLR 2024）：Qwen3-8B (4-bit nf4) 为 teacher，LoRA r=64 蒸馏到 Qwen3-1.7B-Base，GSM8K 准确率从 \_\_% 提到 \_\_%（compute-matched SFT 仅到 \_\_%）。在 Instruct student 上额外验证 OPD 鲁棒性：传统 SFT 触发灾难性遗忘（75→61），OPD 保持原能力（75→76）。λ ∈ {0, 0.5, 1} ablation 验证 on-policy rollout 贡献 \_\_pt。技术栈：TRL 0.21 GKDTrainer / PEFT / bitsandbytes 4-bit / 2× RTX A5000；定位并修复了 Qwen3 thinking 模板导致的 10× 训练降速 bug。"*

跑完 §4.2 base 流程后填三个空。Instruct 数字 (75.4 / 75.7 / 60.7) 已是定值。
